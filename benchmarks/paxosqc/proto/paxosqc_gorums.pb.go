...
func (c *Configuration) Commit(ctx context.Context, in *LearnMsg, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protoqc.PaxosQC.Commit",
	}

	c.RawConfiguration.Multicast(ctx, cd, opts...)
}

// QuorumSpec is the interface of quorum functions for PaxosQC.
...
func (c *Configuration) Prepare(ctx context.Context, in *PrepareMsg) (resp *PromiseMsg, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protoqc.PaxosQC.Prepare",
	}
...
func (c *Configuration) ClientHandle(ctx context.Context, in *Value) (resp *Response, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protoqc.PaxosQC.ClientHandle",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
...
func (srv *Server) Commit(ctx gorums.ServerCtx, request *LearnMsg) {
...
func RegisterPaxosQCServer(srv *Server, impl PaxosQC) {
	srv.RegisterHandler("protoqc.PaxosQC.Prepare", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*PrepareMsg)
		defer ctx.Release()
		resp, err := impl.Prepare(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
	srv.RegisterHandler("protoqc.PaxosQC.Accept", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*AcceptMsg)
		defer ctx.Release()
		resp, err := impl.Accept(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
	srv.RegisterHandler("protoqc.PaxosQC.Commit", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*LearnMsg)
		defer ctx.Release()
		impl.Commit(ctx, req)
	})
	srv.RegisterHandler("protoqc.PaxosQC.ClientHandle", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
