...
// with the same argument in, and returns a combined result.
func (c *Configuration) Learn(ctx context.Context, in *LearnMsg, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protom.PaxosM.Learn",
	}
...
func (c *Configuration) Ping(ctx context.Context, in *Heartbeat, opts ...gorums.CallOption) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protom.PaxosM.Ping",
	}

	c.RawConfiguration.Multicast(ctx, cd, opts...)
}

// QuorumSpec is the interface of quorum functions for PaxosM.
type QuorumSpec interface {
	...
	WriteQF(in *PaxosValue, replies map[uint32]*PaxosResponse) (*PaxosResponse, bool)
...
func (c *Configuration) Write(ctx context.Context, in *PaxosValue) (resp *PaxosResponse, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protom.PaxosM.Write",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
		r := make(map[uint32]*PaxosResponse, len(replies))
		for k, v := range replies {
			r[k] = v.(*PaxosResponse)
		}
		return c.qspec.WriteQF(req.(*PaxosValue), r)
	}
...
func (c *Configuration) Benchmark(ctx context.Context, in *Empty) (resp *Result, err error) {
	cd := gorums.QuorumCallData{
		Message: in,
		Method:  "protom.PaxosM.Benchmark",
	}
	cd.QuorumFunction = func(req protoreflect.ProtoMessage, replies map[uint32]protoreflect.ProtoMessage) (protoreflect.ProtoMessage, bool) {
...
// PaxosM is the server-side API for the PaxosM Service
type PaxosM interface {
	Write(ctx gorums.ServerCtx, request *PaxosValue) (response *PaxosResponse, err error)
	Prepare(ctx gorums.ServerCtx, request *PrepareMsg) (response *PromiseMsg, err error)
	Accept(ctx gorums.ServerCtx, request *AcceptMsg)
	Learn(ctx gorums.ServerCtx, request *LearnMsg)
	Ping(ctx gorums.ServerCtx, request *Heartbeat)
	Benchmark(ctx gorums.ServerCtx, request *Empty) (response *Result, err error)
}

func (srv *Server) Write(ctx gorums.ServerCtx, request *PaxosValue) (response *PaxosResponse, err error) {
...
func (srv *Server) Learn(ctx gorums.ServerCtx, request *LearnMsg) {
...
func RegisterPaxosMServer(srv *Server, impl PaxosM) {
	srv.RegisterHandler("protom.PaxosM.Write", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*PaxosValue)
		defer ctx.Release()
		resp, err := impl.Write(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
	srv.RegisterHandler("protom.PaxosM.Prepare", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
		req := in.Message.(*PrepareMsg)
		defer ctx.Release()
		resp, err := impl.Prepare(ctx, req)
		gorums.SendMessage(ctx, finished, gorums.WrapMessage(in.Metadata, resp, err))
	})
	srv.RegisterHandler("protom.PaxosM.Accept", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*AcceptMsg)
		defer ctx.Release()
		impl.Accept(ctx, req)
	})
	srv.RegisterHandler("protom.PaxosM.Learn", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*LearnMsg)
		defer ctx.Release()
		impl.Learn(ctx, req)
	})
	srv.RegisterHandler("protom.PaxosM.Ping", func(ctx gorums.ServerCtx, in *gorums.Message, _ chan<- *gorums.Message) {
		req := in.Message.(*Heartbeat)
		defer ctx.Release()
		impl.Ping(ctx, req)
	})
	srv.RegisterHandler("protom.PaxosM.Benchmark", func(ctx gorums.ServerCtx, in *gorums.Message, finished chan<- *gorums.Message) {
